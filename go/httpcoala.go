...
func isUserIgnorablePath(p string) bool {
	return strings.Contains(p, "new_items") || (strings.Contains(p, "users") && !strings.Contains(p, "transactions"))
}
...
func (c *coalescer) Route(w http.ResponseWriter, r *http.Request) (*batchWriter, *standbyWriter, bool) {
	...
	if !isUserIgnorablePath(r.URL.Path) {
		return nil, nil, false
	}
