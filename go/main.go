...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		// paging
		err := tx.Select(&items,
			"(SELECT * FROM `items` WHERE (`seller_id` = ? OR `buyer_id` = ?) AND `status` IN (?,?,?,?,?) AND `created_at` = ? AND `id` < ? ORDER BY `id` DESC) UNION ALL (SELECT * FROM `items` WHERE (`seller_id` = ? OR `buyer_id` = ?) AND `status` IN (?,?,?,?,?) AND `created_at` < ? ORDER BY `created_at` DESC) LIMIT ?",
			userId,
			userId,
			ItemStatusOnSale,
			ItemStatusTrading,
